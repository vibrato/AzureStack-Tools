...
if(($Env:AGENT_BUILDDIRECTORY)-and(Test-Path -Path $Env:AGENT_BUILDDIRECTORY)){
    ...
    Write-Host "##vso[task.setvariable variable=TemplateValidatorModule]$((Get-ChildItem -Path $ENV:AGENT_BUILDDIRECTORY -Filter 'AzureRM.TemplateValidator.psm1' -Recurse).FullName)"
}
else {
